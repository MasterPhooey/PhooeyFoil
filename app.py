...
@app.route("/aio", methods=["GET"])
def serve_latest_zip():
    path = os.path.join(OUTPUT_DIR, "latest.zip")
    if not os.path.isfile(path):
        return jsonify({"error": "latest.zip not found."}), 404
    return send_from_directory(OUTPUT_DIR, "latest.zip", as_attachment=True)
